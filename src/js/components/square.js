$.getJSON("assets/data/data.json", function(items) {
    for(var item = 0; item < 28; item++) {
      if (items[item].coverArtworkImage.length > 0) {
        $('#grid').append(
        ...
        for(var item = 0; item < 28; item++) {
          if (items[item].coverArtworkImage.length > 0) {
            ...
            if (item.displayName.search(expression) != -1) {
              if (item.coverArtworkImage.length > 0 && item.listableAppNames.includes('dplus_us') && item.contentSubsetId !== 'atve_us' && item.contentSubsetId !== 'dplus_ca') {
                  $('#grid').append(
